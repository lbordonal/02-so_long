...
/*   Updated: 2023/08/28 19:27:10 by lbordona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	change_enemy_image_w(t_game *game, int x, int y)
{
	mlx_put_image_to_window(
		game->mlx, game->win, game->img_background, x * 32, y * 32);
	mlx_put_image_to_window(
		game->mlx, game->win, game->img_enemy, x * 32, (y - 1) * 32);
...
void	change_enemy_image_a(t_game *game, int x, int y)
{
	mlx_put_image_to_window(
		game->mlx, game->win, game->img_background, x * 32, y * 32);
	mlx_put_image_to_window(
		game->mlx, game->win, game->img_enemy, (x - 1) * 32, y * 32);
	game->map[y][x] = BACKGROUND;
